...
2. `specs/{feature}/spec.md` — 以下のセクションを必ず参照する:
   - **UI Requirements** — コンポーネント階層、レイアウト・スペーシング、状態ごとの表示ルール、アニメーション・トランジション、レスポンシブ挙動、バリデーション表示、空状態・ローディング
   - **Business Rules** — 権限モデル（FE側での表示制御に使用）、状態遷移（ステータスバッジの色・表示切替に使用）
   - **Acceptance Criteria** — UI フロー・操作の検証
...
- [ ] spec.md の Business Rules の権限マトリクスに基づき、ボタン・操作の表示/非表示/disabled が正しく実装されている
