...
```bash
git worktree add -b worktree/scaffold-be-{feature} ../home-hub-be-{feature} HEAD~1
cd ../home-hub-be-{feature}
```

- ブランチ名: `worktree/scaffold-be-{feature}`
...
git checkout worktree/scaffold-be-{feature} -- src/routes/{feature}/
git checkout worktree/scaffold-be-{feature} -- src/lib/server/tables.ts
git checkout worktree/scaffold-be-{feature} -- drizzle/migrations/

# worktree を削除
git worktree remove ../home-hub-be-{feature}
...
BE 実装が完了しました。
次のステップ:
1. 別ターミナルで `/scaffold-fe` を worktree で実行して FE 実装を生成してください。
2. FE も完了したら `/test-and-fix` を実行してテストを GREEN にしてください（unit + integration のみ）。
```
