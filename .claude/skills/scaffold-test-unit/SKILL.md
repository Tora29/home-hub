...
- `.claude/rules/` に testing, schemas が定義されていること
- （推奨）BE 実装コードが存在すること（テスト対象）
...
4. 生成済み BE コード — テスト対象の実装ファイル（存在する場合）
...
| `.claude/rules/schemas.md` | バリデーションルール（異常系テストケースの根拠） |
| `.claude/rules/file-headers.md` | ファイルヘッダーコメントのテンプレートと記述ルール |

### Step 3: AC マッピング

spec.md の AC を分類し、テストケースにマッピングする。
**まず各 AC が BE で検証可能か、FE/E2E でしか検証できないかを判定すること。**

#### AC の性質判定

以下のキーワードを含む AC は **E2E テスト対象**として除外し、テスト生成対象外である旨を出力に記載する:

| キーワード例 | 理由 |
|------------|------|
| 「〜が表示される」「エラーが表示される」 | UI レンダリングの検証 |
| 「〜にリダイレクトされる」 | ページ遷移の検証 |
| 「ボタンをタップすると」「クリックすると」 | ユーザー操作の検証 |
| 「ローディング状態になる」 | UI 状態の検証 |

#### 例外: バリデーションロジックを含む AC

AC が「エラーが表示される」「ボタンをタップすると」などのキーワードを含む場合でも、
その AC が **スキーマバリデーション / ビジネスルール** に対応するものであれば、
バリデーションロジック自体の Unit テストは生成する。

- **バリデーションの部分** → Unit テスト（スキーマ・サービスレベル）として生成
- **UI 表示・操作の部分** → E2E テスト対象として除外し、その旨を出力に記載

例: 「password フィールドが空のままボタンをタップすると『パスワードは必須です』エラーが表示される」
→ `z.string().min(1)` のバリデーション Unit テストを生成 ＋ UI 表示は E2E 対象と記載

上記に該当しない AC を BE テスト（Unit / API）対象とする:
