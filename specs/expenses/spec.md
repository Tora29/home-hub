...
| -------------------- | ---------------------- | -------------------------------------------------------------- | ---- |
| Expense              | 支出                   | id, userId, amount, categoryId, payerUserId, status, createdAt | -    |
| ExpenseWithRelations | 支出（リレーション付） | Expense + category, payer                                      | -    |
| Category             | カテゴリ               | id, userId, name, createdAt                                    | -    |
| User                 | ユーザー               | id, name, email                                                | -    |
...
| 条件                                        | 期待する動作                                                 | 対応AC          |
| ------------------------------------------- | ------------------------------------------------------------ | --------------- |
| user.role が未設定（null）                  | DB 更新は継続。LINE 通知先を解決できないため通知はスキップ   | AC-119          |
...
| month パラメータの月部分が不正（2026-13等） | 400 VALIDATION_ERROR（API）/ 302 リダイレクト（ページ）      | AC-002c, AC-121 |
| 同一ユーザーの checked が 0 件で request    | 409 CONFLICT                                                 | AC-115          |
| 承認対象パートナーの pending が 0 件        | 409 CONFLICT                                                 | AC-118          |
| LINE_CHANNEL_ACCESS_TOKEN 未設定            | DB 更新をコミット、通知スキップ                              | AC-119          |
| 通知先 LINE ユーザー ID 未設定              | DB 更新をコミット、通知スキップ                              | AC-125          |
...
- AC-020: 支出行に金額（¥プレフィックス+カンマ区切り、例: ¥1,000）・カテゴリ名・ステータスバッジ（unapproved:「未承認」、checked:「確認済み」、pending:「申請中」、approved:「承認済み」）が表示される
...
| AC-115〜120                                                                                          | Unit        | `[id]/+server.test.ts`, `service.integration.test.ts`          | request/cancel/approve の CONFLICT・BAD_GATEWAY と関連制約を検証                    | `b0e3fc0b` |
| AC-004〜005                                                                                          | Integration | `service.integration.test.ts`                                  | check/uncheck のステータス遷移・他ユーザー操作を実 D1 で検証                        | `495b580c` |
| AC-004〜007（UI層）                                                                                  | Unit        | `components/ExpenseItem.svelte.test.ts`                        | チェックボックス・編集・削除コールバックの呼び出しを検証                            | —          |
| AC-015〜017                                                                                          | Unit        | `page.svelte.test.ts`, `components/ExpenseItem.svelte.test.ts` | approved/pending 行のグレーアウト・ボタン非表示を検証（コンポーネント単体でも検証） | `7017dde3` |
| AC-018（openMenuId制御）                                                                             | Unit        | `components/ExpenseItem.svelte.test.ts`                        | openMenuId によるメニュー表示制御・onMenuToggle コールバックを検証                  | —          |
| AC-018〜019（モバイルviewport）                                                                      | E2E         | `e2e/expense.e2e.ts`                                           | モバイル viewport 依存のため E2E で検証（testing.md 参照）                          | `04456c16` |
| AC-020                                                                                               | Unit        | `components/ExpenseItem.svelte.test.ts`                        | 金額フォーマット・カテゴリ名・ステータスバッジ表示を検証                            | —          |
