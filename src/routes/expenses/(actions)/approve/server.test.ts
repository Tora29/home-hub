/**
 * @file テスト: API 一括承認
 * @module src/routes/expenses/(actions)/approve/server.test.ts
 * @testType unit
 *
 * @target ./+server.ts
 * @spec specs/expenses/spec.md
 * @covers AC-010, AC-118, AC-120
 */
...
vi.mock('$lib/server/db', () => ({
	createDb: vi.fn().mockReturnValue({
		...
		get: vi.fn().mockResolvedValue({ role: 'partner' })
	})
}));
...
const mockLocals = { user: { id: 'user-2' } };
const mockPlatform = {
	env: {
		DB: {},
...
function makePostRequest(): Request {
	return new Request('http://localhost/expenses/approve', { method: 'POST' });
}
...
describe('POST /expenses/approve', () => {
	describe('正常系', () => {
		test('[SPEC: AC-010] 相手の pending 支出が存在する場合、200 と更新件数を返す', async () => {
			vi.mocked(service.approveExpenses).mockResolvedValueOnce({ count: 4 });
	...
	describe('異常系', () => {
		test('[SPEC: AC-118] 承認対象の pending 支出が 0 件の場合、409 CONFLICT を返す // spec:102bbfbf', async () => {
			vi.mocked(service.approveExpenses).mockRejectedValueOnce(
